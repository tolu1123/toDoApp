//VARIABLES
let password = document.querySelector('#firstPassword');
...
let button = document.querySelector('.resetPassword');
...
confirmPassword.addEventListener('input', () => {
    if(password.value === confirmPassword.value) {
        ...
        warning.textContent = `The Password Field should be equal to the Confirm Password Field`;    
    }
});
...
form.addEventListener('submit', function submit(e) {
    ...
    if(!capital || !lower || !special || !length || !number) {
        e.preventDefault();
    }
});
